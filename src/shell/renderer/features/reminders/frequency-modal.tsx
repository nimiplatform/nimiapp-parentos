...
const PRESET_OPTIONS = [
  { months: 1, labelKey: 'Reminders.frequency.preset.monthly' },
  { months: 3, labelKey: 'Reminders.frequency.preset.quarterly' },
  { months: 6, labelKey: 'Reminders.frequency.preset.semiannual' },
  { months: 12, labelKey: 'Reminders.frequency.preset.annual' },
  { months: 24, labelKey: 'Reminders.frequency.preset.biennial' },
] as const;
...
export function FrequencyModal({ childId, ruleId, ruleTitle, currentIntervalMonths, canDisable = true, existingOverride: existingOverrideProp, onSaved, onClose }: FrequencyModalProps) {
  ...
  const effectiveCurrent = existingOverride?.interval || currentIntervalMonths;
  ...
  useEffect(() => {
    if (!loaded) return;
    const eff = existingOverride?.interval || currentIntervalMonths;
  ...
  if (!loaded || selected === null) {
    return (
      ...
      >
        <span className="text-[14px] text-[var(--nimi-text-muted)]">{i18nText('Reminders.page.loading')}</span>
      </OverlayShell>
    );
  }
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (canDisable && selected === 'disable') {
        await saveFreqOverride(childId, ruleId, { cadenceUnit: 'month', interval: currentIntervalMonths, disabled: true });
      } else {
        ...
        await saveFreqOverride(childId, ruleId, { cadenceUnit: 'month', interval: months, disabled: false });
      }
      onSaved();
      onClose();
    } catch { /* bridge */ }
  ...
  return (
    <OverlayShell
      ...
      title={
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[16px] font-bold text-[var(--nimi-text-primary)]">{i18nText('Reminders.frequency.title')}</h2>
          </div>
      ...
      footer={
        <div className="flex gap-2">
          ...
          >
            {saving ? i18nText('Reminders.action.saving') : i18nText('Reminders.action.confirm')}
          </Button>
          {isCustomized && (
            <Button onClick={() => void handleResetDefault()} disabled={saving} tone="secondary" size="md">
              {i18nText('Reminders.action.restoreDefault')}
            </Button>
          )}
        </div>
      }
    >
      ...
      <p className="mb-4 text-[13px] text-[var(--nimi-text-muted)]">
        {i18nText('Reminders.frequency.defaultDescription', { months: currentIntervalMonths })}
        {isCustomized && !isDisabled && <span className="text-[var(--nimi-action-primary-bg)]"> {i18nText('Reminders.frequency.customizedSuffix', { months: effectiveCurrent })}</span>}
        {isDisabled && <span className="text-[var(--nimi-status-danger)]"> {i18nText('Reminders.frequency.disabledSuffix')}</span>}
      </p>
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        {PRESET_OPTIONS.map((opt) => (
          <button key={opt.months} onClick={() => setSelected(opt.months)}
            ...
            {i18nText(opt.labelKey)}{opt.months === currentIntervalMonths ? i18nText('Reminders.frequency.defaultMarker') : ''}
          </button>
        ))}
        <button onClick={() => { setSelected('custom'); if (!customMonths) setCustomMonths(String(effectiveCurrent)); }}
          ...
          {i18nText('Reminders.frequency.custom')}
        </button>
        {canDisable && (
          <button onClick={() => setSelected('disable')}
            ...
            {i18nText('Reminders.frequency.disable')}
          </button>
        )}
      </div>
      ...
      {selected === 'custom' && (
        <div className="mb-4 flex items-center gap-2">
          <TextField
            ...
            placeholder={i18nText('Reminders.frequency.customPlaceholder')}
            className="w-20"
            inputClassName="text-[14px]"
          />
          <span className="text-[14px] text-[var(--nimi-text-muted)]">{i18nText('Reminders.frequency.monthUnit')}</span>
        </div>
      )}
      ...
      {selected === 'disable' && (
        <p className="mb-4 parentos-radius-md bg-[color-mix(in_srgb,var(--nimi-status-danger)_8%,var(--nimi-surface-card))] px-3 py-2 text-[13px] text-[var(--nimi-status-danger)]">
          {i18nText('Reminders.frequency.disableWarning')}
        </p>
      )}
    </OverlayShell>
  );
