...
export default function ReminderSettingsPage() {
  ...
  if (!child) return <div className="p-8 text-[var(--nimi-text-muted)]">{t('Settings.common.addChildFirst')}</div>;
  ...
  return (
    <div className="min-h-full bg-transparent p-6">
      <div className="mx-auto max-w-3xl">
        ...
        <h1 className="mb-2 text-[24px] font-bold text-[var(--nimi-text-primary)]">{t('Settings.reminders.title')}</h1>
        <p className="mb-6 text-[14px] text-[var(--nimi-text-muted)]">{t('Settings.reminders.desc')}</p>

        {loading ? (
          <p className="text-[14px] text-[var(--nimi-text-muted)]">{t('Settings.common.loading')}</p>
        ) : entries.length === 0 ? (
          <Surface tone="card" material="solid" elevation="base" padding="lg" className="parentos-radius-xl text-center">
            <span className="text-[24px]">⏱️</span>
            <p className="mt-2 text-[14px] font-medium text-[var(--nimi-text-primary)]">{t('Settings.reminders.emptyTitle')}</p>
            <p className="mt-1 text-[13px] text-[var(--nimi-text-muted)]">{t('Settings.reminders.emptyDesc')}</p>
          </Surface>
        ) : (
            ...
            {entries.map((entry) => (
              <Surface key={entry.ruleId} tone="card" material="solid" elevation="base" padding="md" className="flex items-center gap-4 parentos-radius-xl">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="rounded-full bg-[var(--nimi-action-secondary-bg)] px-2 py-0.5 text-[12px] text-[var(--nimi-text-muted)]">
                      {t(`Settings.domains.${entry.domain}`, { defaultValue: entry.domain })}
                    </span>
                  ...
                  <p className="mt-1 text-[13px] text-[var(--nimi-text-muted)]">
                    {t('Settings.reminders.defaultInterval', { months: entry.defaultInterval })}
                ...
                <Button onClick={() => void handleReset(entry.ruleId)} tone="secondary" size="sm" className="shrink-0 text-[13px]">
                  {t('Settings.reminders.restoreDefault')}
                </Button>
              </Surface>
            ))}
          </div>
