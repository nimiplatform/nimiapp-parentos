    ...
    <Timeline>
      {recordGroups.map(([date, dayRecords], gi) => (
        <TimelineGroup
          ...
          secondaryLabel={i18nText('Dental.history.recordsCount', { count: dayRecords.length })}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginTop: 14 }}>
          {attachments.map((a, idx) => (
            <button
              ...
              aria-label={i18nText('Dental.history.photoAria', { index: idx + 1, total: attachments.length })}
