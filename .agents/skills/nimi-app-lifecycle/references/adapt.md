...
Make Kit's `configureNimiElectronAppHostProfile(app)` from `@nimiplatform/kit/shell/electron/host-profile` the first call of the Electron main, before any session, window or `whenReady` work, and load the rest of Kit afterwards, as the generated Host does with `await import('@nimiplatform/kit/shell/electron/main')`. It binds user data, session data and temp to the Host profile Desktop prepared under the selected Nimi data root for both development and installed launches; without that profile the Host must stop rather than use Electron's default location. Do not override those paths with a fixed `app.setPath(...)` or another `--user-data-dir`: Desktop discovers the ephemeral development CDP port from that profile's `DevToolsActivePort` file. Keep business databases and media in their declared App-owned storage, separately from the Electron profile. A `local-development-cdp-port-unavailable` error can mean that the Host exited early or moved that profile; a DevTools listening line alone does not establish successful supervision. Inspect the actual Host startup error and retry the normal launcher after fixing it. Also verify icons, preload and business-process resource paths from the built Host entry, since bundling can change their directory base.

Browser localStorage and IndexedDB are Host-profile data, not the SDK's Nimi-mediated JSON/asset partition. Development and installed launches receive separate Host profiles, and switching the Nimi data root starts from a new one, so do not promise automatic transfer of browser drafts between them. For `app-owned-os-storage`, disclose the actual stable OS roots the packaged Host uses for its own data; its Electron profile stays in the Nimi Host profile. Do not put Nimi's private Host-profile layout in an App manifest. Keep important projects in the App's declared durable storage and use its normal project export/open workflow across those separate profiles; verify restart retention before promising it.
